...
> **Status: scaffold, plus the first frontend feature.** This repository contains the
> project skeleton, the stack decision, the local development harness, the database schema,
> its tenant isolation and the two Prisma clients that enforce it — and, on the frontend,
> the design-token and component foundation plus the agent/team/role management console
> (TAR-82). Feature work is tracked as the TAR-18 epic. The database has **tables but no
> rows**: the data model landed with TAR-47, row-level security with TAR-48, the client
> split with TAR-49, and seed data arrives with TAR-46. The console currently reads
> fixtures rather than the API — see
> [Interim state](#interim-state-mock-api-and-stubbed-role). Everything below works today.
