...
import { TENANT_HOST_HEADER } from '@whatsappcrm/contracts';
...
describe('apiRequest', () => {
  ...
  it('lets a caller override the forwarded host, as it can any other header', async () => {
    ...
    await apiRequest({
      ...
      headers: { [TENANT_HOST_HEADER]: 'acme.example.com' },
    });

    const [, init] = vi.mocked(globalThis.fetch).mock.calls[0] ?? [];

    expect(init?.headers).toMatchObject({ [TENANT_HOST_HEADER]: 'acme.example.com' });
  });
});
