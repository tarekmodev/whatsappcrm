...
function sendMessage({ principal, params, body, headers }: RouteContext): MessageResponse {
  ...
  const created: MockMessage = {
    ...
    // A person typed this one — the composer is the only caller.
    origin: 'agent',
...
function listKnowledgeDocuments({
  principal,
  query,
}: RouteContext): CursorPage<KnowledgeDocumentResponse> {
  ...
  const items = tenantKnowledgeDocuments(principal)
    ...
    // of documents each holding up to 256 KiB is a response nobody wants.
    .map((document) => ({ ...toKnowledgeDocumentResponse(document), content: '' }));
  ...
  const botExchange = tenantMessages(principal)
    .filter(
      (message) =>
        ...
        message.sentAt >= handoff.botEngagedAt &&
...
function requestHandoff({ principal, params, body }: RouteContext): ConversationResponse {
  ...
  if (!canRequestHandoff(conversation.botState)) {
    return toConversationResponse(conversation);
  }
...
function toAiConfigResponse(
  ...
  return {
    ...
    availableModels: AI_MODEL_CATALOGUE,
  };
}
...
/**
 * The model allowlist with its list prices, as the endpoint publishes them.
 *
 * The prices are ADR 0010's table and the API is their one source: a console
 * that hard-coded them would be quoting a tenant a number nobody owns.
 */
const AI_MODEL_CATALOGUE: AiModelOption[] = [
  {
    id: 'claude-opus-5',
    displayName: 'Claude Opus 5',
    inputPricePerMTokUsd: 5,
    outputPricePerMTokUsd: 25,
    isDefault: AI_DEFAULT_MODEL === 'claude-opus-5',
  },
  {
    id: 'claude-sonnet-5',
