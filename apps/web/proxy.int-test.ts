 ...
 * versions. `apps/web/AGENTS.md` says to verify rather than assume, and PR #56
 ...
 * the one recorded in TAR-149: replace the `/api/*` rewrite with an explicit Route
...
describe('the browser path to the API', () => {
  ...
  /**
   * The reason `/api/*` was excluded from the proxy's matcher before TAR-149:
