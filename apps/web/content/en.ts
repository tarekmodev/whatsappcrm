...
export const content = {
  ...
  /**
   * The platform-operator console (TAR-804) — the surface `Reqta Admin.dc.html`
   * specifies and `/api/v1/admin/*` answers.
   *
   * ## Its reader is not the console's reader
   *
   * `docs/STYLE.md` fixes three readers, and this group is the only one in this
   * file written for the **platform operator**: somebody who runs the platform,
   * reads a terminal and does not work inside any tenant. So it says *tenant*
   * where every other group says *workspace*, it names statuses with the
   * contract's own vocabulary rather than the softened tenant-facing wording in
   * `workspace.statuses`, and it is allowed to name an environment variable or an
   * event id — none of which may appear in copy a tenant user reads.
   *
   * Two vocabularies for one set of statuses is deliberate, not drift. A tenant
   * admin reading `cancelled` is told "Closing", because what they need is what
   * happens next; an operator reading the same row needs the value that is in the
   * column, because their next step is a query or a runbook.
   */
  platformAdmin: {
    consoleName: 'Platform admin',
    /** Names the surface in the browser tab, above the tenant's product name. */
    description: 'Operator console for the platform.',
    ...
    signIn: {
      ...
      description:
        'This console is operated by the platform, not by a tenant. Present a platform admin token to continue.',
      tokenLabel: 'Platform admin token',
      /**
       * Names the variable, which is exactly the kind of detail this reader
       * wants and no tenant user may ever see. It does not say what a valid
       * value looks like — the guard answers one refusal for every failure mode
       * on purpose, and copy that narrowed it would undo that.
       */
      tokenHint: 'One of the entries configured in PLATFORM_ADMIN_TOKEN, without the label.',
      tokenRequiredError: 'Enter a platform admin token',
      submit: 'Continue',
      pending: 'Checking…',
      /** The same sentence for every failure the guard refuses, matching it. */
      rejected: 'That token was not accepted.',
      /**
       * The console cannot run against the fixture transport, and says so
       * instead of failing per action. Names both variables, because the reader
       * is the person who sets them.
       */
      ...
      signOut: 'Sign out',
      signedOutToast: 'Signed out of the platform console',
      /**
       * Said once, where the credential is handed over. An operator holding a
       * shared token should know it is shared before they type it somewhere.
       */
      sharedCredentialNotice:
        'A platform admin token authenticates the whole platform, not a person. Every action you take here is recorded against its label.',
    },

    // --- Tenants ------------------------------------------------------------
    tenants: {
      title: 'Tenants',
      subtitle: 'Look up a tenant, read its lifecycle trail, suspend it or bring it back.',
      lookupHeading: 'Find a tenant',
      lookupDescription: 'By slug — the handle in its platform subdomain.',
      slugLabel: 'Tenant slug',
      slugHint: 'Lowercase letters, digits and hyphens. For example: northwind.',
      slugRequiredError: 'Enter a tenant slug',
      slugInvalidError: 'Use lowercase letters, digits and hyphens, 3 to 40 characters',
      lookupSubmit: 'Open tenant',
      /**
       * The gap, named on the screen it affects rather than only in a comment.
       * The admin API has no tenant list and no tenant read, so a console that
       * showed a table here would either be inventing one or paging a different
       * resource and calling it tenants.
       */
      noListNotice:
        'The admin API exposes no tenant list — every read of a tenant is by slug. A cross-tenant list needs GET /v1/admin/tenants, which does not exist yet.',
      notFoundTitle: 'No tenant with that slug',
      notFoundBody: 'Check the spelling. A slug is fixed when the tenant is provisioned.',
      backToLookup: 'Back to tenants',
    },

    tenant: {
      /**
       * The slug is the `<h1>`: it is what the operator typed, what the API keys
       * on, and what they will paste into psql next. The tenant's *display name*
       * is deliberately absent — no read on the admin surface returns it, and a
       * heading assembled from a name this console has not been given would be
       * the one thing on the screen that was made up.
       */
      subtitle: 'Lifecycle state, the trail behind it, and the operator actions it allows.',
      loading: 'Loading tenant lifecycle',
      stateHeading: 'Lifecycle state',
      stateDescription: 'Read from the newest row of the trail below.',
      statusLabel: 'Status',
      lastChangedLabel: 'Last changed',
      lastChangedBy: (actor: string) => `by ${actor}`,
      /**
       * A tenant provisioned before `lifecycle_events` existed has an empty
       * trail, and the backfill writes `unattributed` rather than inventing an
       * actor. Both are states worth naming instead of rendering a blank.
       */
      ...
      /**
       * Plan, usage and MRR are in the reference design and are **not** here.
       * They exist on GET /v1/tenant/lifecycle, which is authenticated by a
       * tenant session; an operator has a platform token and no session, so
       * there is nothing for this console to read them from.
       */
      planHeading: 'Plan and usage',
      planNotice:
        'The admin API exposes no plan, seat usage or revenue figure. Those fields live on GET /v1/tenant/lifecycle, which is authenticated by a tenant session rather than a platform token.',

      trailHeading: 'Lifecycle trail',
      trailDescription: 'Newest first, including the reason recorded with each operator action.',
      trailCaption: 'Tenant lifecycle events',
      trailEmptyTitle: 'No lifecycle rows',
      trailEmptyBody:
        'Nothing has been recorded against this tenant. Rows are written from provisioning onwards.',
      loadMore: 'Load older events',
      /** The way back from a paged view. Forward-only cursors have no previous. */
      ...
      /** `null` on the very first row of a tenant's life — there was no prior state. */
      transition: (from: string | null, to: string) => (from === null ? to : `${from} → ${to}`),
      noReason: 'None recorded',
      noActor: 'Not attributed',
      occurredAtLabel: 'Recorded',

      // --- Operator actions -------------------------------------------------
      actionsHeading: 'Operator actions',
      actionsDescription: 'Both are recorded against your credential label.',
      suspend: 'Suspend tenant',
      suspendTitle: 'Suspend this tenant?',
      /**
       * Names exactly what is lost, per the destructive-action rule: this is the
       * most destructive thing on the surface short of deletion, and the effect
       * is immediate rather than at the next sign-in.
       */
      suspendBody: (slug: string) =>
        `Agents in ${slug} stop reaching their data on their very next request, including through sessions that are already open. Nothing is deleted, and reactivating restores access immediately.`,
      suspendConfirm: 'Suspend tenant',
      suspendReasonLabel: 'Reason',
      suspendReasonHint: 'Recorded on the trail and never shown to the tenant. Optional.',
      suspendedToast: (slug: string) => `${slug} suspended`,
      reactivate: 'Reactivate tenant',
      reactivateTitle: 'Reactivate this tenant?',
      reactivateBody: (slug: string) =>
        `Access returns to ${slug} on its very next request. No data is re-provisioned — it was never deleted.`,
      reactivateConfirm: 'Reactivate tenant',
      reactivatedToast: (slug: string) => `${slug} reactivated`,
      /**
       * Cancel, delete and provision exist on the API and are not built here.
       * Said on the screen rather than left as an absence somebody re-derives by
       * reading the controller.
       */
      unbuiltActionsNotice:
        'Provisioning, cancelling and scheduling a deletion exist on the admin API and are not on this screen. They destroy or create tenants and want their own confirmation design.',
      /**
       * The one thing in the reference design with nothing behind it. Named on
       * the screen so nobody files it as a missing button.
       */
      impersonateNotice:
    ...
    domains: {
      title: 'Domains',
      subtitle: 'Hostnames a tenant has verified, waiting to be attached at the edge.',
      loading: 'Loading the domain queue',
      queueHeading: 'Activation queue',
      queueDescription:
        'A verified domain sits here until somebody attaches it to the web service. Nothing here is urgent enough to page.',
      caption: 'Verified custom domains',
      filterLabel: 'Queue',
      statuses: {
        verified: 'Waiting',
        live: 'Attached',
      ...
      activatedAtLabel: 'Attached',
      activate: 'Mark attached',
      activateAria: (hostname: string) => `Mark ${hostname} attached`,
      deactivate: 'Mark detached',
      deactivateAria: (hostname: string) => `Mark ${hostname} detached`,
      activatedToast: (hostname: string) => `${hostname} marked attached`,
      deactivatedToast: (hostname: string) => `${hostname} marked detached`,
      /**
       * The button records what the operator did at the edge; it does not do it.
       * Saying so is what stops somebody pressing it and expecting a certificate.
       */
      recordOnlyNotice:
        'These record what you have already done in the hosting dashboard. Attaching the hostname and issuing its certificate is a step outside this console — see docs/runbooks/custom-domains.md.',
      emptyWaitingTitle: 'Nothing waiting',
      emptyWaitingBody: 'No tenant has a verified domain that has not been attached.',
      emptyLiveTitle: 'Nothing attached',
      emptyLiveBody: 'No custom domain has been marked attached yet.',
    },

    // --- Parked webhook events ----------------------------------------------
    webhookEvents: {
      title: 'Webhook events',
      subtitle: 'Put a parked inbound event back in front of the sweeper.',
      replayHeading: 'Replay a parked event',
      replayDescription:
        'Resets one event to received, so the next sweep collects it. Reprocessing is asynchronous — the reset is what this confirms.',
      idLabel: 'Webhook event id',
      /**
       * The query is in the README and in TAR-67's runbook. Naming it here is
       * what makes a console with no list usable rather than a dead end: this
       * reader has psql open.
       */
      idHint:
      ...
      submit: 'Replay event',
      pending: 'Replaying…',
      replayedToast: (id: string) => `Event ${id} reset to received`,
      resultHeading: 'Last replay',
      resultId: 'Event',
       ...
       * `received` waits for a worker that does not exist. That is a fact about
       * the platform rather than about the request, so it is said beside the
       * result rather than treated as a failure.
       */
      providerNotSweptNotice:
        'Only the WhatsApp sweeper runs today. An event from another provider stays at received until a worker for it exists.',
      /**
       * The API has one route here and it is not the start of a browser. Said so
       * nobody reads the missing table as an unfinished screen.
       */
      noListNotice:
        'The admin API exposes no way to list parked events — replaying one is its only route. Find the id in webhook_events first.',
    },

    // --- Shared vocabulary ---------------------------------------------------
    /**
     * The contract's own status names, for the operator. Not shared with
     * `workspace.statuses`, which softens the same seven values for a tenant
     * admin — see this group's opening note for why that is two vocabularies
    ...
    lifecycleActors: {
      ...
      system: 'System',
      /** Written by the backfill for the state each tenant was already in. */
