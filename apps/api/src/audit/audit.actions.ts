...
export const AUDIT_ACTIONS = {
   ...
   * (TAR-170).
   *
   * Registration sends a credential this platform generated to Meta and is what
   * makes a number able to message a business's customers, so both outcomes are
   * audited — the failure especially, because a number that silently cannot send
   * is the state this whole story exists to make visible.
   *
   * The metadata carries the phone number id, the WABA id, whether it was the
   * initial attempt or a retry, the published reason, and Meta's own numeric code
   * and `fbtrace_id` — the handle a support ticket with Meta is opened on.
   * **Never the PIN**, encrypted or not, and never Meta's free-text message,
   * which describes this app's grant and this app's configuration.
   */
