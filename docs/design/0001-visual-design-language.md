...
The band carries `BrandLockup` — the same lockup the top bar and the drawer carry — at
`size="lg"` and **unlinked**: the chrome's copy goes to the inbox, which is the one place
a signed-out visitor cannot go. Unlinked, the wordmark (or the logo's `alt`) is what names
the product, because there is no link's accessible name to carry it.
...
| The product's identity  | `BrandLockup` — logo or wordmark; not the rail's   |
