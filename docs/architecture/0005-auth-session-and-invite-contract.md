...
>
> **Decision:** the tenant host travels in `x-forwarded-host`, and the API honours it only
...
> rendering, `proxy.ts` for the browser's calls through the `/api/*` rewrite. The guard-side
> read is TAR-64's remaining backend work.
>
