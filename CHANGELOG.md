...
- **The browser's own API calls name their tenant too, and the naming is now provable**
  ...
  the server-side transport sends. Both headers are `set`, never appended, so a caller that
  names its own tenant is overwritten rather than spliced. `x-forwarded-host` carries the
  tenant; `x-edge-auth` carries `TRUSTED_PROXY_SECRET`, which is what makes the first
  believable — a forwarded host on its own is a tenant the caller chose. The secret is
  server-only (never `NEXT_PUBLIC_`, which would hand it to every visitor) and read at
  runtime rather than baked into the build, so rotating it is a restart. Verified against
  the installed Next 16.3 on both paths, including that a forged pair from the client is
  replaced and that the value appears in no browser asset.
- **Server-side API calls now name the tenant they are for** (TAR-64) — every call made by
  ...
  silently drops a caller-supplied one. **The API side is still to come**: `HostTenantGuard`
